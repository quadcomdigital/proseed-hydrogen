...
async function getAddresses(storefront: any, token: string) {
  const data: any = await storefront.query(`#graphql
...
export default function AccountAddresses() {
  ...
  const [defaultAddress, setDefaultAddress] = useState(data?.defaultAddress);
  ...
  return (
    <div className="space-y-6">
      ...
      {fetcher.data?.errors?.map((e: any, i: number) => (
      ...
      {addresses.length === 0 && !showForm ? (
        <div className="text-center py-12 bg-white border border-gray-100 rounded-3xl">
          ...
          <p className="text-gray-500 text-sm">Nessun indirizzo salvato.</p>
        </div>
      ) : (
                ...
                {editing ? (
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{addr.cap ? `${addr.cap} ` : ''}{addr.city ? `${addr.city}, ` : ''}{addr.province || ''}</p>
...
function AddressForm({fetcher, action, address, onClose}: {
  ...
  return (
    <fetcher.Form method="post" className="bg-white border border-gray-100 rounded-3xl p-6 space-y-4">
      ...
      <div>
        <label className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1 block">Indirizzo *</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1 block">Città *</label>
        ...
        <div>
          <label className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1 block">Provincia</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1 block">CAP</label>
        ...
        <div>
          <label className="text-xs font-black text-gray-500 uppercase tracking-widest mb-1 block">Telefono</label>
